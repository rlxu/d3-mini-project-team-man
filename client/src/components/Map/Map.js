...
class Map extends Component {
  ...
  mapReduce = tweets => {
    ...
    const emojiFreq = tweets
      .map(t => {
        if (!t.location || t.location.length === 0) {
          return undefined;
        }
        const likely = fuzz.extract(t.location, abbreviations, {
    ...
    return Object.entries(emojiFreq)
      ...
      .map(e => {
        ...
        const [emoji, count] = Object.entries(frequencies).sort(
          (a, b) => b[1] - a[1]
        )[0] || ["", 0];
        const data = {
          emoji,
  ...
  render() {
    ...
    return (
      <div>
        <h3 className="Chart-title">Most Popular Emojis Per State ☝</h3>
